    ...
    filterTable: function (tableselector, value) {
        ...
        jQuery("tbody > tr.firstrow", tableselector).each(function (index) {
            ...
            jQuery.each(value, function (index, word) {
                //die W�rter m�ssen alle vorhanden sein:
...
//        jQuery("#verknuepfte_studiengaenge option[value=" + id + "]").attr("selected", true);
    ...
    deleteItem: function (event, ui) {
        ...
        jQuery('<div style="text-align: center;">Wollen Sie den Datensatz tats�chlich l�schen?<br>' +
                        '<div class="studip_button" onClick="' + "location.href='" + STUDIP.URLHelper.getURL(location.href, {'delete_x': 1, 'item_id': id, 'type': type, 'addDocument': 0}) + "'" + '">l�schen</div> ' +
...
//L�schen aus der Liste im Punkt Personal. Zu jedem Typ des Personals (FSB oder StuKo) gibt es eine Liste.
...
jQuery(function ($) {
    ...
    $('#text-preview').click(function () {
        ...
        if (ids.length === 0) {
            alert('Keine Textbausteine ausgew�hlt');
        } else {
        ...
        if (!erroneous) {
            ...
            $.getJSON(url, data, function (json) {
                if (json === false) {
                    ...
                    if ($.isArray(json) && !json.length && !confirm("Dem ausgew�hlten Studiengang sind keine Textbausteine zugeordnet. Wollen Sie wirklich fortfahren?\nIhre aktuelle Textkombinationen werden gel�scht, wenn Sie fortfahren.")) {
                        return;
                    }
            ...
            alert("Bitte w�hlen Sie sowohl ein Fach als auch einen Abschluss aus.".toLocaleString());
        }
        return false;
    });
    ...
    $('#texteditor-infobox #copy-to a').click(function () {
        ...
        if (count === 0) {
            alert("Sie m�ssen mindestens ein Fach, einen Abschluss oder einen Status ausw�hlen.".toLocaleString());
        } else {
