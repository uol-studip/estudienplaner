...
class ModuleOldenburg implements ModuleRetriever
{
    ...
    static function getAllModules ($filter = false, $type = 1) {
        ...
        $data = DBManager::get()->query(
            ...
            "WHERE type = ".DBManager::get()->quote($type)." " .
    ...
    static function getModulesForUser($user_id, $filter = false) {
        ...
        $stmt = DBManager::get()->prepare("SELECT studiengang_id FROM stg_fsb_rollen
            WHERE user_id = ? AND rollen_typ = 'FSB'");
        $stmt->execute(array($user_id));
        $studiengang_ids = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $stmt = DBManager::get()->prepare("SELECT sem_tree.* FROM struktur_zuordnung
            LEFT JOIN sem_tree USING (sem_tree_id)
            WHERE objekt_id = ?");
         foreach ($studiengang_ids as $st_id) {
            $stmt->execute(array($st_id['studiengang_id']));
