...
class StgProfil extends SORM {
    ...
    public function getInformation($language = null) {
        ...
        foreach (self::getPossibleLanguages() as $lang) {
            ...
            // Noch keine Informationen gespeichert? Dann tempor�re ID verwenden...
    ...
    public function setInformation($informationen) {
        ...
        foreach ($datafield_entries as $datafield_id => $datafield_entry) {
            if (!isset($informationen['datenfelder'][$datafield_id])) {
                continue;
            }
            ...
            foreach (array('kurz', 'lang') as $info_form) {
                if ($info_form === "lang" || PersonalRechte::isRoot()) {
                    ...
                    if (is_array($datafield_entries)) {
                        foreach ($datafield_entries as $datafield_id => $datafield_entry) {
                            if (!isset($sprach_infos[$info_form]['datenfelder'][$datafield_id])) {
                                continue;
                            }
    ...
    public function addAnsprechpartner($StgAnsprechpartner_id) {
        if (!$this->hasPermission()) {
            throw new Exception(_("Sie d�rfen diesen Studiengang nicht bearbeiten."));
            return;
        }
        $db = DBManager::get();
    ...
    public function delete() {
        if ($this->is_deletable()) {
            ...
            throw new Exception(_("Dieses Studiengangprofil darf nicht gel�scht werden, weil noch F�cherkombinationen mit ihm existieren."));
            return false;
        }
    }
    ...
    public function offsetSet($offset, $value) {
        ...
        //Trigger werden danach auf dem aktualisierten Bestand ausgef�hrt.
