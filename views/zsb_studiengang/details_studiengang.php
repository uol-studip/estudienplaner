...
<div id="settings" class="accordion" data-active="<?= Request::int('active_tab', 0) ?>">
    ...
    <h2 id="documents"><?= _("Verkn�pfte �ffentliche Dokumente") ?></h2>
    <div>
        <?
        $headers = array(_("Name"), _("Dateiname"), _("Typ"), _("Datum"), _("Gr��e"));
        $items = array();
        $footer_controls = '<a class="icon_plus" title="'._("Neues Dokument hinzuf�gen").'" style="width: 45%" onClick="jQuery(\'#neues_dokument_window\').dialog({ \'width\': \'50%\', \'title\': \''._("Neues Dokument hinzuf�gen").'\'});"></a>';
                ...
                <? foreach ((array)@$textcombinations[$k] as $c): ?>
                    <li class="ui-widget-content">
                        <span class="options">
                            ...
                            <a href="#" class="restriction selector"><span title="<?= _('zulassungsbeschr�nkt') ?>" <? if (in_array($c['restriction'], array('always', 'b'))) echo 'class="selected"'; ?>>Z</span><span title="<?= _('zulassungsfrei') ?>" <? if (in_array($c['restriction'], array('always', 'f'))) echo 'class="selected"'; ?>>F</span></a>
                ...
                <div id="choices-<?= $l ?>">
                    ...
                    <? foreach ($textbausteine[$l] as $textbaustein): ?>
                        <li class="draggable choice ui-widget-content" id="tb-<?= $textbaustein['textbaustein_id'] ?>">
                            <span class="options">
                                <a href="#" class="add">
                                    <?= _('Eintrag zur Auswahl hinzuf�gen') ?>
                                </a>
                            </span>
...
if (count($profile)) {
    ...
    if ($lastone) {
        $zurueck = '<div style="float: left;"><a class="icon_arr_1left" href="'.URLHelper::getLink("?", array('studienprofil_id' => $lastone->getId())).'" title="'._("zur�ck").'"></a></div>';
    }
    if ($nextone) {
        $vor = '<div style="float: right;"><a class="icon_arr_1right" href="'.URLHelper::getLink("?", array('studienprofil_id' => $nextone->getId())).'" title="'._("vorw�rts").'"></a></div>';
    }
}
$infobox = array(
    ($nav_select ? array("kategorie" => _("Navigation:"),
        ...
        array(
            ...
            array(
                "icon" => "",
                "text" => '<div style="text-align: center;">'.$zurueck.' <a class="icon_arr_1up" href="'.URLHelper::getLink("?", array('studiengang_id' => Request::get('studiengang_id'), 'abschluss_id' => Request::get('abschluss_id'))).'" title="'._("Zur Profil�bersicht").'"></a> '.$vor.'</div>'
            )
        )
    ) : null),
